...
impl CDEngine {
                ...
                match &qt_hazard.presence {
                    ...
                    QTHazPresence::Partial(_) => {
                        if !filter.is_irrelevant(qt_hazard.hkey) {
                            let hazard = &self.hazards_map[qt_hazard.hkey];
    ...
    /// * `haz_holes` - Inner rings of the hazard (free pockets carved out of `haz_shape`).
    ///   When the candidate shape is fully contained within one of these holes, it does NOT
    ///   collide with the hazard. Pass `&[]` for hazards without holes.
    /// * `haz_entity` - The entity inducing the hazard
    pub fn detect_containment_collision(
        &self,
        shape: &SPolygon,
        haz_shape: &SPolygon,
        haz_holes: &[Arc<SPolygon>],
        ...
        let contained = match haz_to_shape_bbox_relation {
            GeoRelation::Surrounding => {
                // Candidate is small and inside the hazard's bbox. If its representative
                // point sits in the hazard's outer AND in one of its holes, the candidate is
                // nesting inside that hole's free pocket -> NOT contained in forbidden region.
                // (Edge intersection has already been ruled out earlier in the pipeline, so the
                // candidate cannot straddle a hole boundary at this point.)
                let p = shape.poi.center;
                let in_outer = haz_shape.collides_with(&p);
                if in_outer && haz_holes.iter().any(|h| h.collides_with(&p)) {
                    false
                } else {
                    in_outer
                }
            ...
            match &qt_haz.presence {
                ...
                QTHazPresence::Partial(_) => {
                    if !collector.contains_key(qt_haz.hkey) {
                        let hazard = &self.hazards_map[qt_haz.hkey];
                        if self.detect_containment_collision(
                            shape,
                            &hazard.shape,
                            &hazard.holes,
