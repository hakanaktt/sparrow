...
pub struct Hazard {
    ...
    /// The shape of the hazard (outer boundary)
    pub shape: Arc<SPolygon>,
    /// Optional inner rings (holes) carved out of the hazard. Points inside any of these
    /// rings are considered to be OUTSIDE the hazard's forbidden region. Used so that
    /// items with cut-outs let other items nest inside their pockets.
    /// For static hazards (Exterior, container holes, quality zones) this is empty.
...
impl Hazard {
    ...
    /// Builder-style: attach inner rings to the hazard. See [`Hazard::holes`].
