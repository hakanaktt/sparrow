...
pub struct Importer {
    /// Modification config for "solid" surfaces — item outers, the container outer, and quality zones.
    /// Driven by `min_item_separation` (half goes to items, half to container).
...
impl Importer {
    ...
    /// * `min_item_separation` - Optional minimum separation distance between items and any other
    ///   "solid" hazard (other items, container boundary, quality zones). Half is applied to items
    ///   (Inflate) and half to the container (Deflate). See [`ShapeModifyConfig`].
