...
pub fn layout_to_svg_group(
    ...
    let (items_group, surrogate_group, mut highlight_cd_shape_group) = {
        ...
        for item in instance.items() {
            ...
            // free space — that is Phase B.
            if !item.holes_orig.is_empty() {
                for (i, h_orig) in item.holes_orig.iter().enumerate() {
                    let h_cd = &item.holes_cd[i];
                    item_group = item_group.add(svg_util::data_to_path(
                        svg_util::original_shape_data(h_orig, h_cd, options.draw_cd_shapes),
                        &[
